...
- ◐ **Diff editor** — Monaco side-by-side compare with read-only **compare mode** (HEAD vs Working Tree from the Git panel) + AI-review mode; ✅ **arbitrary file-to-file compare** — editor-tab context menu "Select for Compare" → "Compare with <file>" reads both files and opens the read-only side-by-side diff (`openCompare` in CenterArea, `compareSelection` store slot). ✅ **Merge-conflict resolution** — detects `<<<<<<< / ======= / >>>>>>>` markers in any language, inline CodeLens (Accept Current / Incoming / Both / Compare Changes) + green/blue block decorations & gutter bars (`mergeConflict.ts`, verified: accept-current collapses the block to the chosen side)
...
- ✅ **stash UI** — Source Control page Stashes dropdown: create a stash with an inline message, list all stashes with cleaned labels, and per-stash **Apply** / **Pop** / **Drop**. Rust `git_stash_apply` / `git_stash_pop_index` / `git_stash_drop` (index-addressed, compile-checked); web mock list for preview. Verified: list + drop re-index in the browser.
- ✅ **PR UI** — Source Control "Pull Requests" tab (gh CLI): list open PRs (#number, author, head→base, draft tag), create a PR (title/base/draft) via `gh pr create`, Checkout a PR's branch, Open on GitHub. Rust `gh_pr_list` / `gh_pr_create` / `gh_pr_checkout` / `gh_available` (compile-checked) + web mock list. Verified: list renders, create adds a PR, console clean. ⬜ multiple SCM providers still todo
- ✅ Graphical **settings editor** — Settings dialog reads/writes persisted store values (font size, word wrap, minimap, line numbers, **sticky scroll, bracket/indent guides, font ligatures, render whitespace, cursor blinking** — all apply live + survive reload; also via command-palette toggles; LSP enable + server paths) · **per-workspace `.vscode/settings.json`** applied on folder open (editor.fontSize/wordWrap/minimap/lineNumbers/formatOnSave, files.autoSave)
- ✅ **Keybindings editor (rebindable)** — searchable Keyboard Shortcuts editor (Ctrl+/ or Help menu): COMMAND / KEYBINDING / CATEGORY table with live filter + counts. The 12 app-level commands (`keymap.ts` `APP_COMMANDS`) are **fully rebindable**: click the edit glyph to record a new chord (validated for a Ctrl/Alt modifier + collision-checked), reset-per-row, and reset-all; overrides persist (`keymap` store) and `App.tsx` dispatches via `eventToChord`→effective binding. Monaco/editor/chat bindings shown read-only. Verified: rebind toggles on the new chord, old default ignored, in-editor recording saves + tags "custom", reset clears
...
- ✅ **Multi-root workspaces** — store `workspaceFolders` (persisted) beyond the primary `workspacePath`; Explorer renders each root as a collapsible folder-name section (primary tagged `ROOT`, extras get a remove-folder button), each with its own independent `FileTree`. "Add Folder to Workspace…" in the File menu, command palette, and Explorer toolbar (`addFolderToWorkspace`/`removeFolderFromWorkspace`). Verified via the store debug handle: two roots render with correct headers/badges/files, layout sound, console clean
